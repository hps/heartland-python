...
from securesubmit.entities.credit import HpsCardHolder, HpsCPCData, HpsTrackData
...
class ECommerceTests(unittest.TestCase):
    ...
    test_data = TestData('pkapi_cert_jKc1FtuyAydZhZfbB3')
    ...
    def test_037_fraud_prevention_sale(self):
        response = self.service.charge(15000).with_card(self.test_data.visa_card()).execute()
        self.assertIsNotNone(response)
        self.assertEqual('00', response.response_code)
        self.assertEqual('FR', response.issuser_response_code)

    def test_038_fraud_prevention_return(self):
        response = self.service.refund(15000).with_card(self.test_data.visa_card()).execute()
        self.assertIsNotNone(response)
